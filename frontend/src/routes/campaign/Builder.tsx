...
export function Builder({
  ...
  // Tolerates either key type. JSON stringifies object keys, but a client
  // should not silently compute a spend of zero if that ever changes.
  const cost = (v: number) =>
    rules.costs[String(v)] ?? (rules.costs as Record<number, number>)[v] ?? 0;
